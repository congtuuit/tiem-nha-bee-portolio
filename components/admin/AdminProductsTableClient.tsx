...
import { ChevronLeft, ChevronRight, Edit, Filter, Image as ImageIcon, Loader2, Package, Search, Plus } from "lucide-react";
    ...
    <>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        <div className="relative overflow-x-auto">
          ...
          {/* Mobile view (Cards) */}
          <div className={`md:hidden divide-y divide-slate-100 ${isPending ? "pointer-events-none opacity-60" : ""}`}>
              ...
              return (
                <div key={product.id} className="p-4 flex flex-col gap-3">
                  ...
                  <div className="flex items-center justify-between text-xs pt-2 border-t border-slate-50">
                    <span className={`inline-flex items-center gap-1.5 rounded-md px-2 py-1 font-semibold ${statusClasses}`}>
            ...
            {products.length === 0 ? (
              <div className="py-12 text-center flex flex-col items-center justify-center text-slate-400">
                ...
                <p className="text-base font-medium text-slate-600">Không có sản phẩm</p>
                <Link
                  href="/admin/products/create"
                  className="mt-4 rounded-xl bg-amber-100 px-4 py-2 text-sm font-medium text-amber-700 transition-colors hover:bg-amber-200"
                >
                  Thêm mới
                </Link>
              </div>
          ...
          {/* Desktop view (Table) */}
          <table className="hidden md:table w-full border-collapse whitespace-nowrap text-left">
            <thead>
